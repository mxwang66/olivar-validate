        ...
        with open(primers_path, 'r') as f:
            for line in f:
                try:
                    ...
                    raise ValueError('Each line should only have two values: primer name and primer sequence.')
                primer = msa.attach_primer(seq, temperature, sodium, name)
                out_text += primer.printed + '\n'
    
    if prefix is None:
        if primers_path is None:
            prefix = msa_path
        else:
            prefix = primers_path
